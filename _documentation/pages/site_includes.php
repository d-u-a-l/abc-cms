...
<br>
Для работы с шаблонами у нас есть две функии html_array() и html_query();<br>
...
<h3>Логика размещения шаблонов и использования в них css класов</h3>
<ol>
...
$q - массив данных<br>
$i - номер по порядеку (только в случае с html_query())<br>
$num_rows - количество результатов в выборке (только в случае с html_query())<br>
<pre>&lt;div class="news_list">
	&lt;div class="date">&lt;?=date2($q['date'],'%d.%m.%y')?>&lt;/div>
	&lt;div class="name">
		&lt;a href="/&lt;?=$modules['news']?>/&lt;?=$q['id']?>-&lt;?=$q['url']?>/">&lt;?=$q['name']?>&lt;/a>
	&lt;/div>
	&lt;div class="text">
		&lt;?=$q['text']?>
	&lt;/div>
...
<pre>&lt;div id="news">
	...
	&lt;/div>
&lt;/div></pre>
2) добавлять нужный нтмл код (или клас для блока) в определенном месте (например, каждый третий if (fmod($i,3)==0)<br>
