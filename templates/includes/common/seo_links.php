...
if($_SERVER['REQUEST_URI']) {	$request_url = explode('?',urldecode($_SERVER['REQUEST_URI']));
	...
	//если ссылок меньше 5
	if ($seo_page['display']==1 AND $count<5) {		//новые случайные ссылки, где количество размещений меньше лимита		if ($links_new = mysql_select("
			SELECT sl.*
			FROM seo_links sl
		...
		",'rows_id')) {
			...
			mysql_fn('update','seo_pages',array(
				'id'=>$seo_page['id'],
				'links'=>implode(',',array_keys($links)) //ИД ссылок через запятую
			));
		}
	}
}
