...
class EmailSender {
	...
	public static $_mailer;
        public static $_config = array('driver' =>  'native', 'options' =>  array());
	 ...
	 */
	public static function mailer($config = array('driver' =>  'native', 'options' =>  array()))
	{
		if ( ! EmailSender::$_mailer)
		{
			// Extract configured options
                        $config = array_merge(EmailSender::$_config, $config);
			...
			EmailSender::$_mailer = Swift_Mailer::newInstance($transport);
		}                
		return EmailSender::$_mailer;
	}
	 ...
	 * @return  Email
	 */
	public static function factory($config, $subject = NULL, $message = NULL, $type = NULL)
	{
		return new EmailSender($subject, $message, $type);
	}
	 ...
	 */
	public function send($config = array('driver' =>  'native', 'options' =>  array()) ,array & $failed = NULL)
	{
		return EmailSender::mailer($config)->send($this->_message, $failed);
	}
