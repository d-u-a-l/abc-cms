...
function img_process ($type,$old,$size,$new = '',$quality = 90) {
	//$type - cut - обрезание, resize - уменьшение, bw - черно белый
	//$old  - старый адрес
	//$size - размеры 100x200
	//$new  - новый адрес, если не указан, то используется старый
...
function img_watermark($photo,$logo,$name,$position){ //06.12.09
/* 	ImgImposition('1.jpg','logo.png','newImg.png','all');
	создаст файл newImg.jpg поместим туда изображение из 1.jpg, предварительно замастив его весь ('all') картинкой из logo.png
	вернет treu-если все хорощо или false если не получилось
